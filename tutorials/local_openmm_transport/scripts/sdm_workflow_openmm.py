...
class sdm_job_openmm_asyncre(object):
    ...
    def writeCntlFile(self):
        ...
        #if the job_transport is 'LOCAL_OPENMM', then add the additional information to the asyncre cntl file
        if job_transport == 'LOCAL_OPENMM':
            ...
            r_ids = rcpt_ids
            print(r_ids)
            #convert rcpt atom list to a string using map and join
            rcpt_atom_id = ','.join(map(str,r_ids)) 
            ...
            l_ids = lig_ids
            #convert lig atom list to a string using map and join
            lig_atom_id = ','.join(map(str,l_ids))
            ...
            #MD stepsize and relaxation parameters added for 'LOCAL_OPENMM' transport
